...
namespace _442_a2
{
    public class IndentedStringBuilder
    {
        ...
        private List<string> _endTexts = new List<string>();
        public void AddEndText(string endText)
        {
            // Remove all end texts
            foreach (string text in _endTexts)
            {
                _stringBuilder.Remove(_stringBuilder.Length - text.Length, text.Length);
            }

            // Add the new end text and append all end texts in order
            _endTexts.Add(endText);
            foreach (string text in _endTexts)
            {
        ...
        {
            if (_endTexts.Count > 0)
            {
                // Remove the last end text
                string lastEndText = _endTexts[_endTexts.Count - 1];
                _stringBuilder.Remove(_stringBuilder.Length - lastEndText.Length, lastEndText.Length);

                // Remove the last end text from the list
                _endTexts.RemoveAt(_endTexts.Count - 1);
            }
        }
        ...
        {
            foreach (string endText in _endTexts)
            {
                _stringBuilder.Remove(_stringBuilder.Length - endText.Length, endText.Length);
            }
            ...
            foreach (string endText in _endTexts)
            {
                _stringBuilder.Append(endText);
            }
        }
        public void Append(string text,int space)
        {
            foreach (string endText in _endTexts)
            {
                _stringBuilder.Remove(_stringBuilder.Length - endText.Length, endText.Length);
    ...
    public class Assemblygenerator<T> : IAstVisitor<T>
    {
        ...
        private int freefunctioncallvalue = 0;
        ...
        private List<symbole> currentcallparameter=new List<symbole>();
        public Assemblygenerator(globaltable t) {
            table = t;
            registerpoll= new Analyzer.Stack<register>();
            for (int i = 0; i < 15; i++) {
                registerpoll.Push(new register("r"+i,0));
            } 
        }
        ...
        public void generatefreefunc()
        {
            for(int j = 0; j < table.functiontables.Count - 1; j++)
            {
                if (table.functiontables[j].Name == "main")
                {
                    continue;
                }

                for (int i = 0; i < table.functiontables[j].functionparams.Count; i++)
                {
                    table.functiontables[j].functionparams[i].tag=currentcallparameter[i].tag;
                }
                
                currentfunctiontable = table.functiontables[j];
                stackOffset = 8;
                ...
                assemblyCode.Append(currentfunctiontable.Name+freefunctioncallvalue, 0);
                assemblyCode.AppendLine("sw -4(r14),r15");
                foreach (var i in currentfunctiontable.statements)
                {
                    var x = i as AstNode<T>;
                    x.Accept(this);
                }
                assemblyCode.AppendLine("lw r15,-4(r14)");
                assemblyCode.AppendLine("jr r15");
            }
        }
        public void generatemain()
        {
            ...
            { 
                ...
                else if (i.Type == "integer")
                {
                    if (i.Isarray)
                    {
                        ...
                        //i.stack = stackOffset;
                        //stackOffset -= i.size;
                        i.tag = i.Symbol+currenttempvalue++;
                        assemblyCode.AddEndText($"{i.tag} res {i.size}\n");
                    }
                    else
                    {
                        i.size = 4;
            ...
            currentinofregister = registerpoll.Pop();
        ...
        public void Visit(function<T> function)
        {
            ...
            assemblyCode.AppendLine($"jl r15,{function.Children[0].Value+""+freefunctioncallvalue}");
            assemblyCode.AppendLine($"subi r14,r14,{stackOffset}");
            generatefreefunc();
            freefunctioncallvalue++;
        }
